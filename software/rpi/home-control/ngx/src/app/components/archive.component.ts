...
export class ArchiveComponent implements OnInit, OnDestroy {
    ...
    public refreshNowButtonConfig: ISyncButtonConfig;
    ...
    constructor (private dataService: DataService, private historyService: HistoryService) {
        ...
        for (const t of Object.getOwnPropertyNames(ArchiveComponent.chartTypNames)) {
            typOptions.push(ArchiveComponent.chartTypNames[t]);
        }
        this._inputTyp = {
            id: 'typ',
            disabled: false,
        ...
        for (const a of Object.getOwnPropertyNames(ArchiveComponent.chartAverageNames)) {
            averageOptions.push(ArchiveComponent.chartAverageNames[a]);
        }
        this._inputAverage = {
            id: 'average',
            disabled: false,
        ...
        this._inputMonth = {
            ...
            disabled: false,
            firstLine: 'Monat...',
            options: ArchiveComponent.monthNames,
    ...
    public async onButtonClick(cfg: ISyncButtonConfig): Promise<void> {
        if (cfg === this.refreshNowButtonConfig) {
            this.refresh();
        }
    }
    ...
    private updateValidators (ts: Date) {
        ...
        this._inputMonth.validator.value = ArchiveComponent.monthNames[ts.getMonth()];
    ...
    private refresh () {
        try {
            ...
            if (this.charts.length > 0) {
                const p = Object.assign({}, this.charts[0].params);
                switch (p.average) {
                    case 'minute': {
    ...
    private getType (name: string): TChartTyp {
        ...
        const attributes = Object.getOwnPropertyNames(ArchiveComponent.chartTypNames);
        let rv: TChartTyp = <TChartTyp>attributes.find( (item) => name === ArchiveComponent.chartTypNames[item]);
        if (!rv) {
    ...
    private getAverage (name: string): TChartAverage {
        ...
        const attributes = Object.getOwnPropertyNames(ArchiveComponent.chartAverageNames);
        let rv: TChartAverage = <TChartAverage>attributes.find( (item) => name === ArchiveComponent.chartAverageNames[item]);
        if (!rv) {
    ...
    private handleInputChange (el: ValidatorElement<string>, name: string, newValue: string) {
        ...
        const p = this.charts.length > 0 ? this.charts[0].params : null;
        let t: TChartTyp;
        let a: TChartAverage;
        let dStr: string;
        let mStr: string;
        ...
        const m = ArchiveComponent.monthNames.findIndex( (item) => item === mStr);
    ...
    private async refreshChart (p?: IChartParams) {
        ...
        try {
            ...
            const chart = this.charts.length > 0 ? this.charts[0] : null;
            if (p.typ !== null && p.average !== null) {
                console.log('1--->', chart, p);
                this._locked = p;
                if (p.typ === 'power') {
                    switch (p.average) {
                        case 'minute': {
    ...
    private async createChartPowerMinute (p: IChartParams, ids?: (StatisticAttribute) []): Promise<INg4Chart> {
        ids = ids || ArchiveComponent.defaultPowerAttributes;
        ...
        const chart1Options: Charts.ChartOptions = {
            ...
            title: {
                display: true,
                text: 'Minutenleistung ' + ArchiveComponent.dayNames[from.getDay()] + ', ' +
                       from.getDate() + '. ' + ArchiveComponent.monthNames[from.getMonth()] + ' ' + from.getFullYear()
            },
            scales: {
                xAxes: [{
                    type: 'time',
    ...
    private async createChartPowerHour (p: IChartParams, ids?: StatisticAttribute []): Promise<INg4Chart> {
        ...
        const chart1Options: Charts.ChartOptions = {
            ...
            title: {
                display: true,
                text: 'Tagesleistung ' + ArchiveComponent.dayNames[from.getDay()] + ', ' +
                       from.getDate() + '. ' + ArchiveComponent.monthNames[from.getMonth()] + ' ' + from.getFullYear()
            },
            scales: {
                yAxes: [{
    ...
    private async createChartPowerMonth (p: IChartParams, ids?: StatisticAttribute []): Promise<INg4Chart> {
        ...
        const chart1Options: Charts.ChartOptions = {
            responsive: true,
            title: { display: true, text: 'Monatsleistung ' + ArchiveComponent.monthNames[from.getMonth()] + ' ' + from.getFullYear() },
            scales: {
                xAxes: [{
                    // gridLines: {
